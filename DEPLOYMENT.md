...
`@adguard/text-encoding` is deployed as an **npm package** and a
**GitHub Release** on the public mirror. There is no server, database, or
runtime infrastructure — the library is consumed by other packages via
the npm registry.
...
repository `AdguardTeam/text-encoding`. GitHub Releases are created on
the public mirror.
...
- **Package name**: `@adguard/text-encoding`
...
- **Repository**: `AdguardTeam/text-encoding` (public mirror)
- **Format**: release created automatically on the public mirror
- **Artifact**: `text-encoding.tgz` is attached to each release
...
(`AdguardTeam/text-encoding`) on every push to `master`, using the
...
| Trigger | Purpose |
|---|---|
| `workflow_dispatch` | Manual trigger with tag input (e.g., `v0.9.0`) |
...
| `build` | `team-extensions` | Inject version, lint and test via Docker, build `text-encoding.tgz` |
...
| # | Who | Action |
|---|-----|--------|
| 1 | Developer | Go to **Actions → Prepare release → Run workflow**, enter tag (e.g., `v0.9.0`) |
| 2 | CI | Opens PR: `release-bump/v0.9.0` → `master` with finalized `CHANGELOG.md` |
| 3 | Reviewer | Review the PR body (shows changelog section), approve, merge |
| 4 | CI | `publish-release.yml` auto-fires: creates tag `v0.9.0` → inject → lint → test → build → npm publish → mirror + GitHub Release → Slack |
...
| **Product name** | `@adguard/text-encoding` |
...
enabled for `AdGuardSoftwareLimited/ext-text-encoding`.

**GitHub Release draft is empty**

The release notes are extracted from `CHANGELOG.md`. Ensure the heading
matches the version number exactly in bracket format (e.g.,
`## [0.9.0]` or `## [0.9.0] - 2026-06-18`).
...
`v0.9.0` or a commit SHA). This manually triggers the release pipeline.
